...
Provides advanced search features including fuzzy search, link queries, historical related news retrieval, etc.
"""
...
class SearchTools:
    ...
    def search_news_unified(
        ...
        Args:
            ...
            date_range: Date range (optional)
                       ...
                       - **Default**: If not specified, defaults to querying today
                       - **Note**: start and end can be the same (single day query)
            platforms: Platform filter list, e.g. ['coindesk', 'cointelegraph']
            ...
            sort_by: Sort method, options:
                ...
                - "date": Sort by date
            threshold: Similarity threshold (only effective for fuzzy mode), 0-1, default 0.6
            include_url: Whether to include URL links, default False (saves tokens)

        Returns:
            Search result dictionary containing matching news list

        Examples:
            - search_news_unified(query="Bitcoin", search_mode="keyword")
            - search_news_unified(query="Ethereum price drop", search_mode="fuzzy", threshold=0.4)
            - search_news_unified(query="Vitalik Buterin", search_mode="entity", limit=20)
            - search_news_unified(query="Base chain", date_range={"start": "2025-01-01", "end": "2025-01-07"})
        """
        try:
            ...
            if date_range:
                ...
                # When date is not specified, use latest available data date (not datetime.now())
            ...
            # Build time range description (correctly determine if today)
            if start_date.date() == datetime.now().date() and start_date == end_date:
    ...
    def _search_by_entity_mode(
        ...
        for platform_id, titles in all_titles.items():
            ...
            for title, info in titles.items():
                # Entity search: exact match of entity name
                if query in title:
                    news_item = {
                        "title": title,
                        "platform": platform_id,
    ...
    def _extract_keywords(self, text: str, min_length: int = 2) -> List[str]:
        ...
        # Word segmentation using regex (supports multiple languages)
    ...
    def search_related_news_history(
        ...
        try:
            ...
            if len(all_related_news) < limit:
                result["note"] = f"Only found {len(all_related_news)} related news items with similarity threshold {threshold}"
