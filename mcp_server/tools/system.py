...
class SystemManagementTools:
    ...
    def trigger_crawl(self, platforms: Optional[List[str]] = None, save_to_local: bool = False, include_url: bool = False) -> Dict:
        ...
        Args:
            platforms: Specified platform list, if empty then crawl all platforms
        ...
        Returns:
            Crawl result dictionary, including news data and save paths (if saved)

        Example:
            >>> tools = SystemManagementTools()
            >>> # Temporary crawl, don't save
            >>> result = tools.trigger_crawl(platforms=['coindesk', 'cointelegraph'])
            >>> print(result['data'])
            >>> # Crawl and save to local
            >>> result = tools.trigger_crawl(platforms=['coindesk'], save_to_local=True)
        ...
        try:
            ...
            # Use DataFetcher for crawling (supports new and legacy platforms)
            try:
                import sys
                from pathlib import Path
                ...
                proxy_url = None
                if use_proxy:
                    proxy_url = config_data.get("crawler", {}).get("default_proxy", "")
                
                # Create DataFetcher instance
                fetcher = DataFetcher(proxy_url=proxy_url if proxy_url else None)
                
                # Crawl data
                results, id_to_name, failed_ids = fetcher.crawl_websites(ids, request_interval)
                
            except ImportError as e:
                # If import fails, fallback to old method
                print(f"Warning: Cannot import DataFetcher, using fallback method: {e}")
                results = {}
                id_to_name = {}
                failed_ids = []
            ...
            if save_to_local:
                try:
                    ...
                    result["note"] = "Data has been persisted to output folder"
...
"""
        ...
        if failed_ids:
            ...
            html += '            <h3>Failed Platform Requests</h3>\n'
            html += '            <ul>\n'
