...
def calculate_news_weight(news_data: Dict, rank_threshold: int = 5) -> float:
    ...
    Based on the weight algorithm in main.py, comprehensively considers:
    - Rank weight (60%): News ranking in the list
    - Frequency weight (30%): Number of times the news appears
    ...
    # 1. Rank weight: Σ(11 - min(rank, 10)) / number of appearances
    rank_scores = []
    for rank in ranks:
    ...
    # 2. Frequency weight: min(appearances, 10) × 10
    frequency_weight = min(count, 10) * 10

    # 3. Hotness bonus: high ranking count / total appearances × 100
...
class AnalyticsTools:
    ...
    def analyze_data_insights_unified(
        ...
        Examples:
            - analyze_data_insights_unified(insight_type="platform_compare", topic="Bitcoin")
    ...
    def analyze_topic_trend_unified(
        ...
        Args:
            ...
            analysis_type: Analysis type, options:
                - "trend": Trend analysis (track topic hotness changes)
                - "lifecycle": Lifecycle analysis (complete cycle from appearance to disappearance)
                - "viral": Viral detection (identify suddenly trending topics)
                - "predict": Topic prediction (predict future potential hotspots)
            ...
            threshold: Hotness surge multiplier threshold (viral mode), default 3.0
            time_window: Detection time window in hours (viral mode), default 24
            lookahead_hours: Prediction future hours (predict mode), default 6
        ...
        Examples:
            - analyze_topic_trend_unified(topic="Bitcoin", analysis_type="trend", date_range={"start": "2025-10-18", "end": "2025-10-25"})
            - analyze_topic_trend_unified(topic="Base", analysis_type="lifecycle", date_range={"start": "2025-10-18", "end": "2025-10-25"})
            - analyze_topic_trend_unified(topic="Ethereum", analysis_type="viral", threshold=3.0)
            - analyze_topic_trend_unified(topic="DeFi", analysis_type="predict", lookahead_hours=6)
        """
        try:
            # Parameter validation
    ...
    def get_topic_trend_analysis(
        ...
        """
        Trend analysis - track hotness change trends for specific topics
        ...
        Examples:
            User query examples:
            - "Analyze the trend of 'Bitcoin' in the past week"
            - "View 'Ethereum' hotness changes in the past week"
            - "Check 'Base' trends in the last 7 days"
            - "Analyze 'DeFi' hotness trends in the past month"
            - "View 'Web3' trend changes in December 2024"
            ...
            ...     granularity="day"
            ... )
            >>> # Analyze historical monthly trend
            >>> result = tools.get_topic_trend_analysis(
            ...     topic="Base",
        ...
        try:
            ...
            if granularity != "day":
                ...
                raise InvalidParameterError(
                    ...
                    suggestion="Currently only supports 'day' granularity, as underlying data is aggregated by day"
                )

            # Process date range (defaults to last 7 days if not specified)
            if date_range:
            ...
            while current_date <= end_date:
                try:
                    ...
                    # Count topic occurrences at this time point
                    count = 0
                    matched_titles = []
                    ...
                    trend_data.append({
                        ...
                        "count": count,
                        "sample_titles": matched_titles[:3]  # Keep only first 3 samples
                    })
    ...
    def compare_platforms(
        ...
        Args:
            topic: Topic keyword (optional, if not specified, compares overall activity)
            date_range: Date range, format: {"start": "YYYY-MM-DD", "end": "YYYY-MM-DD"}

        Returns:
            Platform comparison analysis result

        Examples:
            User query examples:
            - "Compare each platform's attention to 'Bitcoin' topic"
            - "See which platform between CoinDesk and Cointelegraph pays more attention to crypto news"
            - "Analyze today's hotspot distribution across platforms"
        ...
        try:
            ...
            # Find unique hotspots for each platform
    ...
    def analyze_keyword_cooccurrence(
        ...
        Examples:
            ...
            - "See which words 'Bitcoin' often appears with"
                    ...
                    if len(keywords) >= 2:
                        for i, kw1 in enumerate(keywords):
                            for kw2 in keywords[i+1:]:
                                # Sort uniformly to avoid duplicates
    ...
    def analyze_sentiment(
        ...
        Examples:
            ...
            - "See if 'Bitcoin' related news is positive or negative"
            - "Analyze each platform's sentiment towards 'Base'"
            - "See if 'Ethereum' related news is positive or negative, please select the top 10 news from the past week for analysis"

            Code call examples:
            >>> tools = AnalyticsTools()
            >>> # Analyze today's Bitcoin news, return top 10
            >>> result = tools.analyze_sentiment(
            ...     topic="Bitcoin",
            ...     limit=10
            ... )
            >>> # Analyze Bitcoin news from the past week, return top 10 sorted by weight
            >>> result = tools.analyze_sentiment(
            ...     topic="Bitcoin",
        ...
        try:
            ...
            # Deduplicate (keep only one instance of same title)
            unique_news = {}
            ...
            # If returned count is less than requested, add note
            if len(selected_news) < limit and len(deduplicated_news) >= limit:
                result["note"] = "Returned count is less than requested due to deduplication logic (same title across different platforms is kept only once)"
    ...
    def _create_sentiment_analysis_prompt(
        ...
        Args:
            news_data: News data list (sorted and limited)
            topic: Topic keyword
        ...
        # 1. Task description
        if topic:
            prompt_parts.append(f"Please analyze the sentiment tendency of the following news headlines about '{topic}'.")
        else:
            prompt_parts.append("Please analyze the sentiment tendency of the following news headlines.")

        prompt_parts.append("")
        prompt_parts.append("Analysis requirements:")
        prompt_parts.append("1. Identify the sentiment tendency of each news item (positive/negative/neutral)")
        ...
        prompt_parts.append("4. Summarize overall sentiment trends")
        ...
        prompt_parts.append("")
        prompt_parts.append("## Overall Sentiment Trends")
    ...
    def find_similar_news(
        ...
        Examples:
            User query examples:
            - "Find news similar to 'Bitcoin price drop'"
            - "Find similar reports about Ethereum launch"
            - "See if there are similar reports to this news"

            Code call examples:
            >>> tools = AnalyticsTools()
            >>> result = tools.find_similar_news(
            ...     reference_title="Bitcoin announces price drop",
            ...     threshold=0.6,
            ...     limit=10
            ... )
        ...
        try:
            ...
            if not result_items:
                raise DataNotFoundError(
                    ...
                    suggestion="Please lower the similarity threshold or try other titles"
                )
            ...
            if len(similar_items) < limit:
                result["note"] = f"Only found {len(similar_items)} similar news items with similarity threshold {threshold}"
    ...
    def search_by_entity(
        ...
        Args:
            ...
            limit: Return count limit, default 50, max 200
        ...
        Examples:
            User query examples:
            - "Search for news related to 'Vitalik Buterin'"
            - "Find reports about Coinbase company, return top 20"
            - "See what news there is about 'Base'"
    ...
    def generate_summary_report(
        ...
        """
        Daily/Weekly summary generator - automatically generate hotspot summary reports
...
"""
            ...
            if report_type == "weekly":
                ...
                for keyword in top_keywords:
                    markdown += f"- **{keyword}**: Continuously hot\n"
            ...
            if all_titles_list:
                ...
                # Sort by weight descending, if same weight then by title alphabetical order (ensures determinism)
            ...
            markdown += "\n---\n\n*This report is automatically generated by BaseRadar MCP*\n"
    ...
    def get_platform_activity_stats(
        ...
        """
        Platform activity statistics - Statistics on publishing frequency and active time periods for each platform

        Args:
            date_range: Date range (optional)

        Returns:
            Platform activity statistics result

        Examples:
            User query examples:
            - "Statistics on today's activity for each platform"
        ...
        try:
            ...
            # Statistics on platform activity
            ...
            while current_date <= end_date:
                try:
                    ...
                    for platform_id, titles in all_titles.items():
                        ...
                        # Statistics on time distribution (based on time in filename)
    ...
    def analyze_topic_lifecycle(
        ...
        Args:
            ...
            date_range: Date range (optional)
                       - **Format**: {"start": "YYYY-MM-DD", "end": "YYYY-MM-DD"}
                       - **Default**: If not specified, defaults to last 7 days
        ...
        Examples:
            ...
            - "Track the heat changes of 'Bitcoin' topic"
        ...
        try:
            ...
            while current_date <= end_date:
                try:
                    ...
                    # Count topic occurrences for this day
                    count = 0
                    for _, titles in all_titles.items():
                        for title in titles.keys():
                            if topic.lower() in title.lower():
    ...
    def detect_viral_topics(
        ...
        """
        Abnormal heat detection - Automatically identify suddenly viral topics

        Args:
            threshold: Heat surge multiplier threshold
        ...
        Examples:
            ...
            - "See if there are any abnormally hot news"
            - "Alert for possible major events"
        ...
        try:
            ...
            if not viral_topics:
                return {
                    ...
                    "viral_topics": [],
                    "total_detected": 0,
                    "message": f"No topics detected with heat growth exceeding {threshold}x"
                }
    ...
    def predict_trending_topics(
        ...
        """
        Topic prediction - Predict future possible hot topics based on historical data
        ...
        Returns:
            Predicted potential topics list
        ...
        try:
            ...
            # Collect last 3 days of data for prediction
            ...
            for keyword, trend_data in keyword_trends.items():
                ...
                if growth_rate > 0.3:  # Growth exceeds 30%
                    ...
                    if len(trend_data) >= 3:
                        # Check if it's consistently growing
                        is_consistent = all(
                    ...
                    if confidence >= confidence_threshold:
                        predicted_topics.append({
                            ...
                            "trend_data": trend_data,
                            "prediction": "Upward trend, may become hot topic",
            ...
            return {
                ...
                "note": "Prediction based on historical trends, actual results may vary"
            }
    ...
    def _extract_keywords(self, title: str, min_length: int = 2) -> List[str]:
        ...
        # Simple word segmentation (by spaces and common separators)
    ...
    def _find_unique_topics(self, platform_stats: Dict) -> Dict[str, List[str]]:
        """
        Find unique hotspot topics for each platform
