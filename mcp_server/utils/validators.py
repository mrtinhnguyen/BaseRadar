...
def get_supported_platforms() -> List[str]:
    ...
    Note:
        - Returns empty list on read failure, allowing all platforms to pass (degradation strategy)
...
def validate_platforms(platforms: Optional[List[str]]) -> List[str]:
    ...
    Note:
        ...
        - Validates if platform IDs are in config.yaml platforms configuration
        - When configuration loading fails, allows all platforms to pass (degradation strategy)
    """
...
def validate_config_section(section: Optional[str]) -> str:
    ...
    """
    valid_sections = ["all", "crawler", "notification", "keywords", "weights"]
...
def validate_date_query(
    ...
    Args:
        ...
        max_days_ago: Maximum days allowed to query
    ...
    if not date_query:
        raise InvalidParameterError(
            ...
            suggestion="Please provide date query, e.g.: today, yesterday, 2025-10-10"
        )
