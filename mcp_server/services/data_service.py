...
class DataService:
    ...
    def get_trending_topics(
        ...
        """
        Get news frequency statistics for personal keywords

        Note: This tool is based on the personal keyword list in config/frequency_words.txt for statistics,
        rather than automatically extracting hot topics from news. Users can customize this keyword list.
        ...
        if mode == "daily":
            ...
            # current mode: only process latest batch data (latest timestamp file)
            if timestamps:
                # Find latest timestamp
                latest_timestamp = max(timestamps.values())

                # Re-read, only get latest time data
                # Here we reverse lookup the platform corresponding to latest file through timestamps dictionary
                latest_titles, _, _ = self.parser.read_all_titles_for_date()

                # Since read_all_titles_for_date returns merged data from all files,
                # we need to filter out latest batch through timestamps
                # Simplified implementation: use current all data as latest batch
    ...
    def get_current_config(self, section: str = "all") -> Dict:
        ...
        Args:
            section: Configuration section - all/crawler/notification/keywords/weights
        ...
        if section == "all" or section == "notification":
            push_config = {
            ...
            if webhooks.get("email_from") and webhooks.get("email_password") and webhooks.get("email_to"):
                push_config["enabled_channels"].append("email")
