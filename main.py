...
class DataFetcher:
    ...
    def fetch_data(
        ...
        # Use new crypto crawlers for crypto platforms
        if CRAWLERS_AVAILABLE:
            crypto_platforms = [
                "coindesk", "cointelegraph", "decrypt", "beincrypto",
                "coingape", "cryptonews", "theblock", "coinpedia",
                "base-blog", "mirror-xyz", "base-mirror", "defillama",
                "messari", "airdrops-io", "cryptoslate"
            ]
            if id_value.lower() in crypto_platforms:
                retries = 0
                while retries <= max_retries:
                    try:
                        result = crawl_platform(id_value, self.proxy_url)
                        if result.get("status") == "success":
                            ...
                            data_text = json.dumps(formatted_data, ensure_ascii=False)
                            print(f"Successfully fetched {id_value} (latest data)")
                            return data_text, id_value, alias
                        else:
                            raise ValueError(result.get("message", "Crawl failed"))
                    except Exception as e:
                        retries += 1
        ...
        # Old API for legacy platforms (if any remain)
...
def read_all_today_titles(
    ...
    """Read all title files for today, supports filtering by current monitored platforms"""
    date_folder = format_date_folder()
    txt_dir = Path("output") / date_folder / "txt"
...
def detect_latest_new_titles(current_platform_ids: Optional[List[str]] = None) -> Dict:
    """Detect newly added titles in the latest batch today, supports filtering by current monitored platforms"""
    date_folder = format_date_folder()
    txt_dir = Path("output") / date_folder / "txt"
...
def matches_word_groups(
    ...
    # If no word groups configured, match all titles (supports displaying all news)
    if not word_groups:
        return True
    ...
    for source_id, titles_data in results_to_process.items():
        ...
        for title, title_data in titles_data.items():
            ...
            for group in word_groups:
                ...
                is_new = False
                if all_news_are_new:
                    # In incremental mode all processed news are new, or all news on first time today are new
                    is_new = True
        ...
        if is_first_today:
            ...
            if new_titles:
                ...
                if matched_new_count == 0 and len(word_groups) > 1:
                    print("Incremental mode: No new news matched frequency words, will not send notification")
            else:
...
def prepare_report_data(
    ...
    # Only process new news section in non-hidden mode
...
def generate_html_report(
    ...
    if is_daily_summary:
        if mode == "current":
            filename = "Current_Ranking_Summary.html"
...
def render_email_html_content(
    ...
    """Render HTML content optimized for email clients"""
    now = get_utc_time()
    
    # Determine report type
    if is_daily_summary:
        if mode == "current":
            report_type_text = "Current Ranking Summary"
        elif mode == "incremental":
            report_type_text = "Daily Incremental"
        else:
            report_type_text = "Daily Summary"
    else:
        report_type_text = "Real-time Analysis"
...
<head>
    ...
    <title>BaseRadar Hot News Report</title>
</head>
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: #f5f5f5; line-height: 1.6;">
        ...
        <tr>
            <td align="center" style="padding: 20px 0;">
                <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="600" style="max-width: 600px; background-color: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
                    <!-- Header -->
                    <tr>
                        <td style="background: linear-gradient(135deg, #0052FF 0%, #0052FF 50%, #00D4FF 100%); padding: 40px 30px; text-align: center;">
                            <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%">
                                <tr>
                                    <td style="text-align: center;">
                                        <h1 style="margin: 0 0 30px 0; font-size: 28px; font-weight: 700; color: #ffffff; letter-spacing: -0.5px;">
                                            📡 BaseRadar
                                        </h1>
                                    </td>
                                </tr>
                                <tr>
                                    <td>
                                        <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="background-color: rgba(255, 255, 255, 0.15); border-radius: 8px; padding: 20px;">
                                            <tr>
                                                <td width="50%" style="text-align: center; padding: 10px; border-right: 1px solid rgba(255, 255, 255, 0.2);">
                                                    <div style="font-size: 11px; color: rgba(255, 255, 255, 0.9); text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 6px;">Report Type</div>
                                                    <div style="font-size: 16px; font-weight: 600; color: #ffffff;">{html_escape(report_type_text)}</div>
                                                </td>
                                                <td width="50%" style="text-align: center; padding: 10px;">
                                                    <div style="font-size: 11px; color: rgba(255, 255, 255, 0.9); text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 6px;">Total News</div>
                                                    <div style="font-size: 16px; font-weight: 600; color: #ffffff;">{total_titles} items</div>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td width="50%" style="text-align: center; padding: 10px; border-right: 1px solid rgba(255, 255, 255, 0.2); border-top: 1px solid rgba(255, 255, 255, 0.2);">
                                                    <div style="font-size: 11px; color: rgba(255, 255, 255, 0.9); text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 6px;">Hot News</div>
                                                    <div style="font-size: 16px; font-weight: 600; color: #ffffff;">{hot_news_count} items</div>
                                                </td>
                                                <td width="50%" style="text-align: center; padding: 10px; border-top: 1px solid rgba(255, 255, 255, 0.2);">
                                                    <div style="font-size: 11px; color: rgba(255, 255, 255, 0.9); text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 6px;">Generated</div>
                                                    <div style="font-size: 16px; font-weight: 600; color: #ffffff;">{now.strftime('%m-%d %H:%M')} UTC</div>
                                                </td>
                                            </tr>
                                        </table>
                                    </td>
    ...
    if report_data["failed_ids"]:
        html += """
                            <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="background-color: #fef2f2; border-left: 4px solid #dc2626; border-radius: 4px; padding: 16px; margin-bottom: 24px;">
                                <tr>
                                    <td>
                                        <div style="font-size: 14px; font-weight: 600; color: #dc2626; margin-bottom: 8px;">⚠️ Failed Platform Requests</div>
                                        <div style="font-size: 13px; color: #991b1b;">"""
        for id_value in report_data["failed_ids"]:
            html += f'<div style="padding: 4px 0;">• {html_escape(id_value)}</div>'
        html += """
                                        </div>
                                    </td>
                                </tr>
    ...
    if report_data["stats"]:
        ...
        for i, stat in enumerate(report_data["stats"], 1):
            ...
            html += f"""
                            <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="margin-bottom: 32px; border-bottom: 2px solid #f0f0f0;">
                                <tr>
                                    <td style="padding-bottom: 16px;">
                                        <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%">
                                            <tr>
                                                <td>
                                                    <div style="display: inline-block; background-color: {count_bg}; color: {count_color}; font-size: 12px; font-weight: 700; padding: 4px 10px; border-radius: 12px; margin-right: 10px;">
                                                        {count} items
                                                    </div>
                                                    <span style="font-size: 12px; color: #999;">[{i}/{total_count}]</span>
                                                </td>
                                            </tr>
                                        </table>
                                    </td>
            ...
            for j, title_data in enumerate(stat["titles"], 1):
                ...
                escaped_source = html_escape(title_data["source_name"])
                ...
                if ranks:
                    ...
                    rank_html = f'<span style="display: inline-block; background-color: {rank_bg}; color: #ffffff; font-size: 10px; font-weight: 700; padding: 2px 6px; border-radius: 10px; margin-right: 6px;">{rank_text}</span>'
                
                # Time display
                time_html = ""
                if title_data.get("time_display"):
                    time_display = html_escape(title_data["time_display"].replace(" ~ ", "~").replace("[", "").replace("]", ""))
                    time_html = f'<span style="color: #999; font-size: 11px;">{time_display}</span>'
                
                # Count info
                count_html = ""
                if title_data.get("count", 1) > 1:
                    count_html = f'<span style="color: #059669; font-size: 11px; font-weight: 500;">({title_data["count"]} times)</span>'
                
                new_badge = '<span style="background-color: #fbbf24; color: #92400e; font-size: 9px; font-weight: 700; padding: 2px 6px; border-radius: 4px; margin-left: 8px; text-transform: uppercase; letter-spacing: 0.5px;">NEW</span>' if is_new else ""
                                            ...
                                            <tr>
                                                <td width="30" style="vertical-align: top; padding-right: 12px;">
                                                    <table role="presentation" cellspacing="0" cellpadding="0" border="0">
                                                        <tr>
                                                            <td style="width: 24px; height: 24px; background-color: #f8f9fa; border-radius: 50%; text-align: center; font-size: 13px; font-weight: 600; color: #999; line-height: 24px;">{j}</td>
                                                        </tr>
                                                    </table>
                                                </td>
                                                <td style="vertical-align: top;">
                                                    <div style="margin-bottom: 6px;">
                                                        <span style="font-size: 12px; color: #666; font-weight: 500;">{escaped_source}</span>
                                                        {rank_html}
                                                        {time_html}
                                                        {count_html}
                                                        {new_badge}
                                                    </div>
                                                    <div style="font-size: 15px; line-height: 1.4; color: #1a1a1a;">"""
                
                if link_url:
                    escaped_url = html_escape(link_url)
                    html += f'<a href="{escaped_url}" style="color: #2563eb; text-decoration: none;">{escaped_title}</a>'
                else:
                    html += escaped_title
    ...
    if report_data["new_titles"]:
        html += f"""
                            <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="margin-top: 40px; padding-top: 24px; border-top: 2px solid #f0f0f0;">
                                <tr>
                                    <td>
                                        <div style="font-size: 16px; font-weight: 600; color: #1a1a1a; margin-bottom: 20px;">
                                            🆕 New Hot News (Total {report_data['total_new_count']} items)
                                        </div>"""
        ...
        for source_data in report_data["new_titles"]:
            ...
            html += f"""
                                        <div style="margin-bottom: 24px;">
                                            <div style="font-size: 13px; font-weight: 500; color: #666; margin-bottom: 12px; padding-bottom: 6px; border-bottom: 1px solid #f5f5f5;">
                                                {escaped_source} · {titles_count} items
                                            </div>"""
            ...
            for idx, title_data in enumerate(source_data["titles"], 1):
                ...
                if ranks:
                    ...
                    rank_html = f'<span style="display: inline-block; background-color: {rank_bg}; color: #ffffff; font-size: 10px; font-weight: 700; padding: 3px 6px; border-radius: 8px; margin-right: 8px;">{rank_text}</span>'
                else:
                    rank_html = '<span style="display: inline-block; background-color: #6b7280; color: #ffffff; font-size: 10px; font-weight: 700; padding: 3px 6px; border-radius: 8px; margin-right: 8px;">?</span>'
                                                ...
                                                <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%">
                                                    <tr>
                                                        <td width="20" style="vertical-align: top; padding-right: 8px;">
                                                            <table role="presentation" cellspacing="0" cellpadding="0" border="0">
                                                                <tr>
                                                                    <td style="width: 20px; height: 20px; background-color: #f8f9fa; border-radius: 50%; text-align: center; font-size: 12px; font-weight: 600; color: #999; line-height: 20px;">{idx}</td>
                                                                </tr>
                                                            </table>
                                                        </td>
                                                        <td style="vertical-align: top;">
                                                            {rank_html}
                                                            <span style="font-size: 14px; line-height: 1.4; color: #1a1a1a;">"""
    ...
    html += f"""
                ...
                <!-- Footer -->
                <tr>
                    <td style="padding: 20px 30px; background-color: #f8f9fa; border-top: 1px solid #e5e7eb; text-align: center;">
                        <div style="font-size: 13px; color: #6b7280; line-height: 1.6;">
                            Generated by <strong style="color: #374151;">BaseRadar</strong> · 
                            <a href="https://github.com/mrtinhnguyen/BaseRadar" style="color: #0052FF; text-decoration: none; font-weight: 500;">Made with ❤️ for the Base ecosystem</a>"""
    
    if update_info:
        html += f"""
                            <br><br>
                            <span style="color: #ea580c; font-weight: 500;">
                                New version {update_info['remote_version']} found, current version {update_info['current_version']}
                            </span>"""
...
def render_html_content(
    ...
    # Handle failed ID error information
    if report_data["failed_ids"]:
        html += """
                <div class="error-section">
                    <div class="error-title">⚠️ Failed Platform Requests</div>
                    <ul class="error-list">"""
        for id_value in report_data["failed_ids"]:
    ...
    if report_data["new_titles"]:
        html += f"""
                <div class="new-section">
                    <div class="new-section-title">New Hot News (Total {report_data['total_new_count']} items)</div>"""
    ...
    html += """
            ...
            <div class="footer">
                <div class="footer-content">
                    Generated by <span class="project-name">BaseRadar</span> · 
                    <a href="https://github.com/mrtinhnguyen/BaseRadar" target="_blank" class="footer-link">
        ...
        <script>
            async function saveAsImage() {
                ...
                try {
                    ...
                    const now = new Date();
                    const filename = `BaseRadar_Hot_News_Analysis_${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, '0')}${String(now.getDate()).padStart(2, '0')}_${String(now.getHours()).padStart(2, '0')}${String(now.getMinutes()).padStart(2, '0')}.png`;
            ...
            async function saveAsMultipleImages() {
                ...
                try {
                    ...
                    // Download all images
                    const now = new Date();
                    const baseFilename = `BaseRadar_Hot_News_Analysis_${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, '0')}${String(now.getDate()).padStart(2, '0')}_${String(now.getHours()).padStart(2, '0')}${String(now.getMinutes()).padStart(2, '0')}`;
...
def split_content_into_batches(
    ...
    if format_type == "telegram":
        ...
        if update_info:
            base_footer += f"\nBaseRadar found new version {update_info['remote_version']}, current {update_info['current_version']}"

    stats_header = ""
    # Statistics header not displayed in Telegram messages
    ...
    if report_data["stats"]:
        ...
        for i, stat in enumerate(report_data["stats"]):
            ...
            # Build word group header
            word_header = ""
            # Keywords not displayed in Telegram messages
...
def send_to_notifications(
    ...
    if email_from and email_password and email_to:
        results["email"] = send_to_email(
            ...
            total_titles=total_titles_for_email,
            mode=mode,
...
def send_to_email(
    ...
    try:
        ...
        if report_data is not None:
            # Determine if it's daily summary from report_type
            is_daily_summary = "Summary" in report_type or "Daily" in report_type
            email_mode = mode
            
            html_content = render_email_html_content(
                report_data, total_titles, is_daily_summary, email_mode, update_info
            )
            print(f"Generated email-optimized HTML for {report_type}")
        elif html_file_path and Path(html_file_path).exists():
            # Fallback: read from file if report_data not provided
            print(f"Using HTML file: {html_file_path}")
            with open(html_file_path, "r", encoding="utf-8") as f:
                html_content = f.read()
        else:
            print(f"Error: No HTML content available for email")
            return False
        ...
        # Strictly follow RFC standard for From header
        sender_name = "BaseRadar"
        ...
        # Set email subject
        now = get_utc_time()
        subject = f"BaseRadar Hot News Report - {report_type} - {now.strftime('%m/%d %H:%M')}"
...
class NewsAnalyzer:
    ...
    MODE_STRATEGIES = {
        "incremental": {
            ...
            "description": "Incremental mode (only focuses on new news, no push when there are no new items)",
    ...
    def _check_version_update(self) -> None:
        ...
        try:
            ...
            if need_update and remote_version:
                ...
                print("Version check completed, currently on latest version")
        except Exception as e:
    ...
    def _has_valid_content(
        ...
        if self.report_mode in ["incremental", "current"]:
            # In incremental and current modes, if stats has content, it means there are matched news
    ...
    def _send_notification_if_needed(
        ...
        if (
            ...
            if "Real-time" in report_type:
                print(
                    f"Skipping real-time push notification: No matched news detected in {mode_strategy['mode_name']}"
                )
            else:
                print(
    ...
    def _execute_mode_strategy(
        ...
        # In current mode, real-time push needs to use complete historical data to ensure statistical information integrity
        if self.report_mode == "current":
            ...
            if analysis_data:
                ...
                # Send real-time notification (using statistical results from complete historical data)
                summary_html = None
                ...
                print("❌ Critical error: Unable to read the just saved data file")
        ...
        if mode_strategy["should_generate_summary"]:
            if mode_strategy["should_send_realtime"]:
                # If real-time notification has been sent, summary only generates HTML without sending notification
